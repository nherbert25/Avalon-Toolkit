...
def handle_client(conn, addr):
    ...
    while connected:
        ...
        # lock.locked() shows the visual studio error: "Instance of 'lock' has no 'locked' member" but it's a bug. This method works.
            ...
            print(
                f"Is a lock on? {lock.locked()}. We are releasing the lock immediately after this line if one exists, if there is an error around here, this might be the issue.")
            ...
            print(
                f"Testing if thread is currently locked: Is a lock on? {lock.locked()}")
            if lock.locked() and (threading.activeCount() - 1) == 1:

...
def select_player(player_frame, player_name, player_role, user_board_state_info, username):
    ...
    print('You are ' + username + ', It is currently ' +
          client_board_state.board_state['player_picking_team'] + 's turn')
    print(
        f'Before pressing, these players are selected: {client_board_state.selected_players}')
    ...
    if client_board_state.username == client_board_state.board_state['player_picking_team']:
        ...
        print(
            f'After pressing, these players are selected: {client_board_state.selected_players}')
...
def submit_team(all_player_frames, username, COOL_BLUE, EVIL_RED):
    ...
    if client_board_state.username == client_board_state.board_state['player_picking_team'] and len(client_board_state.selected_players) == client_board_state.board_state['team_size'][0][client_board_state.board_state['round']-1]:
        ...
        client_board_state.client_queue.append(
        ...
        for dictionary in all_player_frames.values():
            ...
            dictionary['player_base_frame_widget'].configure(bg=update_player_widget_background_color(
                dictionary['player_base_frame_widget'], dictionary['player_board_state_info'], username, user_board_state_info, COOL_BLUE, EVIL_RED))
...
# ALL PLAYER INFORMATION FOR WIDGET     XXXX player_widget_dictionary['player_board_state_info']
        ...
        print('active player: ' +
              client_board_state.board_state['player_picking_team'])
        print('# of selected players: ' +
              str(len(client_board_state.selected_players)))
        ...
        print('team size to select: ' +
              str(client_board_state.board_state['team_size'][0][client_board_state.board_state['round']-1]))
...
def approve_succeed_button(all_player_frames=None):
    ...
    if client_board_state.board_state['phase'] == 'mission_phase' and client_board_state.username in client_board_state.board_state['team_selected']:
        client_board_state.client_queue.append(
            ['!MISSION', client_board_state.username, 'pass'])
        print('passing')
...
def reject_fail_button(all_player_frames=None):
    ...
    if client_board_state.board_state['phase'] == 'voting_phase':
        client_board_state.client_queue.append(
            ['!VOTE', client_board_state.username, 'reject'])
        print('rejecting')

    if client_board_state.board_state['phase'] == 'mission_phase' and client_board_state.username in client_board_state.board_state['team_selected']:
        client_board_state.client_queue.append(
            ['!MISSION', client_board_state.username, 'fail'])
        print('failing')
...
def start_game(widget, start_button, list_of_characters):
    ...
    if role_count >= len(client_board_state.players):
        ...
        client_board_state.client_queue.append(
            ['!GAMESTART', client_board_state.roles])
        print('Starting game!')
        # widget.grid_forget()
...
def update_voter_frame(widget_frame, widget_dictionary, approve_color, reject_color, board_state=None):
    ...
    for player in board_state['players']:
        ...
        votes = player['votes']
        # 'on_team': [[True, True, True], [True], [True]]
        on_team = player['on_team']
        # 'made_team': [[False, True, False], [False], [True]]
        ...
        for round, (vote_round_list, on_team_bool_round_list, made_team_bool_round_list) in enumerate(zip(votes, on_team, made_team)):
            ...
            # for vote in round:
                ...
                if vote == 'approve':
                    widget_dictionary[name][widget_index].configure(
                        bg=approve_color, fg=approve_color)
                if vote == 'reject':
                    widget_dictionary[name][widget_index].configure(
                        bg=reject_color, fg=reject_color)

                # config widget text depending if on team or not
                if on_team_bool:
                    widget_dictionary[name][widget_index].configure(
                        text="✔", fg="white", font="BOLD 12")

...
class Main_Page():
    ...
    def __init__(self, lock):
        ...
        self.root.protocol('WM_DELETE_WINDOW', self.crash_gui)
        self.main_frame = tk.LabelFrame(self.root, bd=10, bg=self.NEUTRAL_BLUE)
        self.main_frame.grid(row=0, column=0)

        self.player_name_widget = self.generate_player_name_widget(
            self.main_frame)
        self.player_name_widget.grid(row=0, column=2, sticky='EW')

        self.top_frame = tk.LabelFrame(
            self.main_frame, bg=self.NEUTRAL_BLUE, bd=5, pady=10, padx=5)  # , text="Top Frame")
        ...
        self.player_lobby_widget, self.player_frame_widget_dictionary = self.generate_player_lobby_widget(
            top_frame=self.top_frame, list_of_players=Main_Page.list_of_players)
    ...
    def generate_player_lobby_widget(self, top_frame, list_of_players):
        ...
        config_base_frame = tk.LabelFrame(
            top_frame, bg=self.NEUTRAL_BLUE, bd=1)  # , text="Player Frame")
        ...
        for player_name in list_of_players:
            ...
            player_base_frame_widget = tk.LabelFrame(
                config_base_frame, bg=self.NEUTRAL_BLUE, bd=5, padx=10, pady=10)
            player_base_frame_widget.grid(
                row=row_num, column=column_num)  # , sticky='EW')

            player_text_widget = tk.Label(
                player_base_frame_widget, font=40, text=player_name, height=5, width=10)
            player_text_widget.grid(row=0, column=0, sticky='EW')
            # ZZZ

            player_frame_widget_dictionary[player_name] = {
                'player_base_frame_widget': player_base_frame_widget, 'player_text_widget': player_text_widget}
            column_num += 1
    ...
    def generate_rules_config(self):
        ...
        config_base_frame = tk.LabelFrame(
            self.main_frame, bg=self.NEUTRAL_BLUE, bd=10)  # , text="Rules Frame")
        config_base_frame.grid(row=1, column=0, sticky='EW')

        characters_widget = tk.Label(
            config_base_frame, font=40, text=main_board_helper.list_of_characters, pady=2)
        ...
        for character in main_board_helper.list_of_characters:
            ...
            def f_factory(character=character):
                ...
                add_char_button = tk.Button(config_base_frame, text=character, font=40,
                                            command=lambda: main_board_helper.char_add(character, characters_widget))
                add_char_button.grid(row=0, column=count)
                remove_char_button = tk.Button(config_base_frame, text=character, font=40,
                                               command=lambda: main_board_helper.char_remove(character, characters_widget))
        ...
        start_game_button = tk.Button(config_base_frame, text='Start!', font=40, command=lambda: main_board_helper.start_game(
            config_base_frame, start_game_button, main_board_helper.list_of_characters))
    ...
    def update_player_lobby_widget_to_start_game(self, top_frame, player_frame_widget_dictionary, board_state, username):
        ...
        for player_name, player_widget_dictionary in player_frame_widget_dictionary.items():
            ...
            player_base_frame_widget.config(bg=main_board_helper.update_player_widget_background_color(
                player_base_frame_widget, player_board_state_info, username, user_board_state_info, self.COOL_BLUE, self.EVIL_RED))

            # update widget text (currently this updates the base widget color as well, trying to seperate this)
            player_text_widget.configure(text=main_board_helper.playerframetext(
                player_base_frame_widget, player_board_state_info, username, user_board_state_info))
            ...
            if 'select_player_button' not in player_frame_widget_dictionary[player_name].keys():
                ...
                def create_select_player_button(player_frame=player_base_frame_widget, player_name=player_board_state_info['name'], player_role=player_board_state_info['role']):
                    ...
                    select_player_button = tk.Button(player_base_frame_widget, text='Select!', bg=self.GOOD_BLUE, fg=self.GOOD_BLUE_TEXT_COLOR, font=(
                        'Helvetica', '8'), command=lambda: main_board_helper.select_player(player_frame, player_name, player_role, user_board_state_info, username))
        ...
        config_base_frame_submit = tk.LabelFrame(
            top_frame, bg=self.NEUTRAL_BLUE, bd=0)  # , text="Submit Frame")
        config_base_frame_submit.grid(row=1, column=0)

        submit_button = tk.Button(config_base_frame_submit, text='Submit!', bd=5, bg=self.GOOD_BLUE, fg=self.GOOD_BLUE_TEXT_COLOR, font=(
            'Helvetica', '12'), command=lambda: main_board_helper.submit_team(player_frame_widget_dictionary, username, self.COOL_BLUE, self.EVIL_RED))
        submit_button.grid(row=0, column=0, pady=(10, 10))

        # create Vote Buttons frame widget
        # , text="config_base_frame_vote")#, sticky='NSEW')
        config_base_frame_vote = tk.LabelFrame(
            top_frame, bg=self.NEUTRAL_BLUE, bd=0)
        ...
        approve_button_labelframe_widget = tk.LabelFrame(
            config_base_frame_vote, bg=self.NEUTRAL_BLUE, bd=0)  # , text='omg')#, padx=10, pady=10)
        ...
        reject_button_labelframe_widget = tk.LabelFrame(
            config_base_frame_vote, bg=self.NEUTRAL_BLUE, bd=0)  # , text='bruh')#, padx=10, pady=10)
        ...
        approve_succeed_button = tk.Button(approve_button_labelframe_widget, width=10, text='Approve\r\nSucceed', bd=5, relief="raised",
                                           bg=self.GOOD_BLUE, fg=self.GOOD_BLUE_TEXT_COLOR, font=('Helvetica', '12'), command=lambda: main_board_helper.approve_succeed_button())
        # , sticky='nsew') #padx=(0, 20),
        approve_succeed_button.grid(row=0, column=0)
        approve_succeed_button.grid_columnconfigure(0, weight=1)
        #approve_succeed_button.grid(row=0, column=0, sticky='EW')
        #player_text_widget.grid(row=0, column=0, sticky='EW')

        reject_fail_button = tk.Button(reject_button_labelframe_widget, width=10, text='Reject\r\nFail', bd=5, bg=self.EVIL_RED,
                                       fg=self.GOOD_BLUE_TEXT_COLOR, font=('Helvetica', '12'), command=lambda: main_board_helper.reject_fail_button())
        # , sticky='nsew')#padx=(20, 0),
        reject_fail_button.grid(row=0, column=1)
    ...
    def generate_team_score_widget(self, top_frame, team_size):
        ...
        team_widget_list = []
        config_base_frame = tk.LabelFrame(
            top_frame, bg=self.NEUTRAL_BLUE, bd=0, pady=15, padx=5)
        ...
        for team, two_fail in zip(team_size[0], team_size[1]):
            ...
            team_frame_widget = tk.Label(config_base_frame, bg=self.NEUTRAL_BLUE, bd=5,
                                         padx=6, pady=3, relief="raised", text=team, font='Helvetica 18 bold')
            team_frame_widget.grid(row=0, column=column_num)  # , sticky='EW')
            team_frame_widget.grid_columnconfigure(column_num, weight=1)

            team_number_text_widget = tk.Label(
                team_frame_widget, text=team, font='Helvetica 18 bold', bg=self.NEUTRAL_BLUE)  # , height=1, width=1)
            # team_number_text_widget.grid(row=0, column=0)#, sticky='EW')

            # , team_number_text_widget])
            team_widget_list.append(team_frame_widget)
            column_num += 1
    ...
    def generate_server_message_frame(self, message_from_server=None):
        ...
        config_base_frame = tk.Label(
            self.main_frame, bg=self.NEUTRAL_BLUE, font=40, bd=10, text=message_from_server, pady=2)
    ...
    def generate_voting_frame(self):
        config_base_frame = tk.LabelFrame(
            self.main_frame, bg=self.NEUTRAL_BLUE, bd=0)  # , text="Voting Frame")
        config_base_frame.grid(row=2, column=0)  # , fill=tk.X)

        vcount = 0
        xcount = 1
        for i in range(1, 6):
            # , font=12)  bg=self.NEUTRAL_BLUE,
            round_title = tk.Label(
                config_base_frame, text=f'Round {i}', font="Helvetica 14 bold italic", bd=1, relief='raised')
            round_title.grid(row=vcount, column=xcount,
        ...
        for player in client_board_state.board_state['player_order']:
            ...
            # if player=='Frankie':
            player_name = tk.Label(config_base_frame, text=player,
                                   font="Helvetica 12 bold", bd=0, bg=self.NEUTRAL_BLUE, relief='raised')
            ...
            for i in range(25):
                ...
                xcount += 1
                # , borderwidth=2)#,, highlightbackground="#37d3ff", highlightthickness=4)
                votebox = tk.Label(config_base_frame, text="✔", fg="white",
                                   font="BOLD 12", relief="solid", bd=3, bg='white')
    ...
    def main_loop(self):
        ...
        if client_board_state.board_state['phase'] != 'lobby_phase' and self.game_phase == 'lobby_phase':
            ...
            self.player_lobby_widget, self.player_frame_widget_dictionary = self.generate_player_lobby_widget(
                top_frame=self.top_frame, list_of_players=Main_Page.list_of_players)
            self.player_frame_widget_dictionary = self.update_player_lobby_widget_to_start_game(
                top_frame=self.top_frame, player_frame_widget_dictionary=self.player_frame_widget_dictionary, board_state=client_board_state.board_state, username=client_board_state.username)

            # create and update team score frame
            self.team_score_widget, self.team_widget_list = self.generate_team_score_widget(
                self.top_frame, client_board_state.board_state['team_size'])
            self.update_team_score_widget(self.team_score_widget, self.team_widget_list,
                                          client_board_state.board_state['mission'], self.GOOD_BLUE, self.EVIL_RED)
            ...
            main_board_helper.update_voter_frame(
                self.voting_frame, self.player_vote_dictionary, Main_Page.GOOD_BLUE, Main_Page.EVIL_RED)
        ...
        if client_board_state.message_from_server != Main_Page.message_from_server:
            print(
                f"Updating message from {client_board_state.message_from_server} to {Main_Page.message_from_server}.")
            Main_Page.message_from_server = client_board_state.message_from_server
            self.server_message_frame.configure(
                text=Main_Page.message_from_server)
